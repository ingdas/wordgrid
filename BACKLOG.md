...
- `Deduction.tsx` + `deductionLevels.ts` — 🧩 Logic Grid: a pure-logic mode,
  **20 levels across Easy/Medium/Hard tiers**. 12 words hidden in a 3×4 or 4×3
  grid; 4 themes × 3 tiles, each a connected region. **Only some tiles show a
  clue number** (how many orthogonal neighbours share that tile's theme) — the
  rest are blank; you deduce and colour all 12. Fewer shown clues = harder
  (Easy ~8-10, Medium ~6-8, Hard ~4-6 of 12). Every level is solvable by pure
  deduction from its shown clues alone — no vocabulary/memory/timer — so it has
  one solution. Paint with a 4-colour brush; a full-but-wrong board shakes; a
  prev/next stepper pages the 20 (tier chip shown); solving flips the tiles to
  reveal the words + hidden link. Pre-generated by `scripts/gen-deduction.mjs`
  (`npm run gen:deduction`): enumerates 3×4 & 4×3 partitions, keeps only grids a
  **constraint-propagation solver cracks with zero guesses**, then greedily
  **minimises the shown clues** to the fewest that stay no-guess-solvable (padded
  per tier), asserting uniqueness-from-shown-clues. `progress.deductionSolved`
  tracks cleared ids (+500 each).
- `StartScreen.tsx` — Daily hero card (7-day streak strip, countdown,
  Solve CTA), full-width Continue·L{n}, then an Endless · Pairs · Logic mode
  row (each with its best/count), rank/XP bar (playerRank).
