...
- `Deduction.tsx` + `deductionLevels.ts` — 🧩 Logic Grid: a pure-logic mode,
  **20 levels across Easy/Medium/Hard tiers**. 12 words hidden in a 3×4 or 4×3
  ...
  Paint with a 4-colour brush; a full-but-wrong board shakes and **rings the
  violated clue tiles**; a prev/next stepper pages the 20 (tier chip); solving
  flips the tiles to reveal the words + hidden link. Pre-generated by
