...
- `Deduction.tsx` + `deductionLevels.ts` — 🧩 Logic Grid: a pure-logic mode,
  **20 levels**. 12 words hidden in a 3×4 or 4×3 grid; each tile shows only a
  NUMBER = how many orthogonal neighbours share its (hidden) theme. 4 themes ×
  3 tiles, each a connected region. Every level is solvable by pure deduction —
  no vocabulary/memory/timer — so it has one solution. Paint tiles with a
  4-colour brush; a full-but-wrong board shakes; a prev/next stepper pages the
  20 puzzles; solving flips the tiles to reveal the words + hidden link. Levels
  are pre-generated by `scripts/gen-deduction.mjs` (`npm run gen:deduction`),
  which enumerates every 3×4 & 4×3 partition, keeps only clue grids a
  **constraint-propagation solver can crack with zero guesses** (8 vetted
  templates), drapes 20 boards' words over them, and asserts uniqueness +
  no-guess solvability. `progress.deductionSolved` tracks cleared ids (+500 each).
