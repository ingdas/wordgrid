...
- `Deduction.tsx` + `deductionLevels.ts` — 🧩 Logic Grid: a pure-logic mode.
  12 words hidden in a 3×4 grid; each tile shows only a NUMBER = how many
  orthogonal neighbours share its (hidden) theme. 4 themes × 3 tiles, each a
  connected region, and the clue grid has a **unique** solution — so you reach
  it by reasoning, no vocabulary/memory/timer. Paint tiles with a 4-colour
  brush; a full-but-wrong board shakes; solving flips the tiles to reveal the
  words + hidden link. Levels are pre-generated by `scripts/gen-deduction.mjs`
  (`npm run gen:deduction`), which enumerates every 3×4 partition, keeps only
  clue grids with one solution, and drapes a board's words over them; it
  asserts uniqueness. `progress.deductionSolved` tracks cleared ids (+500 each).
